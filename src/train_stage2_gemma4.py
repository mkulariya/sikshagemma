...
DEFAULT_DATASET_PATH = "sikshagemma_stage2_training.jsonl"
DEFAULT_OUTPUT_DIR = "stage2_gemma4_e4b"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Stage 2 Gemma-4 E4B – COLLATOR FIX")
...
def load_stage2_dataset(dataset_path, sample_size=0):
...
def train(args):
    ...
    dataset = load_stage2_dataset(args.dataset_path, args.sample_size)
    formatted = format_conversations(dataset, tokenizer)
    ...
    save_run_metadata(
        os.path.join(args.output_dir, "stage2_run_summary.json"),
        {
...
if __name__ == "__main__":
    ...
    if args.analyze_seq_len:
        dataset = load_stage2_dataset(args.dataset_path, sample_size=args.sample_size)
